...
def editArticles(id):
    ...
    if form.validate_on_submit():
        ...
        db.session.add(article)
        db.session.commit()
        flash(u'Blog updated successfully！', 'success')
...
def manage_articles():
    ...
    form.types.choices = types
    sources = [(s.id, s.name) for s in Source.query.all()]
    sources.append((-1, u'All sources'))
    form.source.choices = sources
...
def delete_article():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            flash(u'Successful deletion of blog posts and %s comments!' % count, 'success')
    if form.errors:
        flash(u'Delete failed！', 'danger')
...
def delete_articles():
    ...
    if form.validate_on_submit():
        ...
        try:
            ...
            flash(u'Successfully delete %s blog posts and %s comments!' % (len(articleIds), count), 'success')
    if form.errors:
        flash(u'Delete failed！', 'danger')
...
def disable_comment(id):
    ...
    db.session.commit()
    flash(u'Screen comment success！', 'success')
...
def enable_comment(id):
    ...
    db.session.add(comment)
    db.session.commit()
    flash(u'Successful Resume Display Comments！', 'success')
...
def delete_comment(id):
    ...
    try:
        ...
        flash(u'Delete comment successful！', 'success')
...
def manage_comments():
    ...
    if form.validate_on_submit():
        ...
        db.session.add(comment)
        db.session.commit()
        flash(u'Submit comments successfully！', 'success')
        return redirect(url_for('.manage_comments'))
    if form.errors:
        flash(u'Failed to submit comments! Please check if there are any errors in the filling.', 'danger')
        return redirect(url_for('.manage_comments'))
...
def delete_comments():
    ...
    if form2.validate_on_submit():
        ...
        try:
            ...
            flash(u'Successful deletion of %s comment!' % count , 'success')
    if form2.errors:
        flash(u'Delete failed！', 'danger')
...
def manage_articleTypes():
    ...
    if form.validate_on_submit():
        ...
        if articleType:
            flash(u'Failed to add classification! The classification name already exists.', 'danger')
        else:
            ...
            db.session.commit()
            flash(u'Add classification success！', 'success')
        return redirect(url_for('.manage_articleTypes'))
    if form.errors:
        flash(u'Failed to add classification! Please check if there are any errors in the filling.', 'danger')
        return redirect(url_for('.manage_articleTypes'))
    ...
    if form2.validate_on_submit():
        ...
        if articleType.is_protected:
            if form2.name.data != articleType.name or \
                ...
                flash(u'You can only modify the system default classification attributes and navigation!', 'danger')
            else:
                ...
                db.session.add(articleType)
                db.session.commit()
                flash(u'Successful modification of system default classification！', 'success')
            ...
            and ArticleType.query.filter_by(name=form2.name.data).first().id != articleType_id:
                flash(u'Failed to modify the classification! The classification name already exists.', 'danger')
        else:
            ...
            db.session.add(articleType)
            db.session.commit()
            flash(u'Modified classification success！', 'success')
        return redirect(url_for('.manage_articleTypes', page=page))
    if form2.errors:
        flash(u'Failed to modify the classification! Please check if there are any errors in the filling.', 'danger')
        return redirect(url_for('.manage_articleTypes', page=page))
...
def delete_articleType(id):
    ...
    if articleType.is_protected:
        flash(u'Warning: You do not have the right to delete the default classification of the system!', 'danger')
    ...
    try:
        ...
        db.session.rollback()
        flash(u'Deletion classification failed！', 'danger')
    else:
        flash(u'Delete the classification successfully! At the same time, the% s blog posts of the original classification are added to <Uncategorized>.' % count, 'success')
    return redirect(url_for('admin.manage_articleTypes', page=page))
...
def manage_articleTypes_nav():
    ...
    if form.validate_on_submit():
        ...
        if menu:
            page = page
            flash(u'Failed to add navigation! The navigation name already exists.', 'danger')
        else:
            ...
            db.session.commit()
            page = -1
            flash(u'Added navigation successfully!', 'success')
    ...
    if form2.validate_on_submit():
        ...
        if Menu.query.filter_by(name=name).first() \
            and Menu.query.filter_by(name=name).first().id != nav_id:
                flash(u'Modifying navigation failed! The navigation name already exists.', 'danger')
        else:
            nav = Menu.query.get_or_404(nav_id)
            nav.name = name
            db.session.add(nav)
            db.session.commit()
            flash(u'Successful navigation modification!', 'success')
        return redirect(url_for('admin.manage_articleTypes_nav', page=page))
    if form2.errors:
        flash(u'Modifying navigation failed! Please check if there are any errors in the filling.', 'danger')
        return redirect(url_for('admin.manage_articleTypes_nav', page=page))
...
def delete_nav(id):
    ...
    try:
        ...
        db.session.rollback()
        flash(u'Delete navigation failed!', 'danger')
    else:
        flash(u'Delete navigation successfully! At the same time, the navigation classified by% s of the original navigation is set to none.' % count, 'success')
    return redirect(url_for('admin.manage_articleTypes_nav', page=page))
...
def nav_sort_up(id):
    ...
    if pre_menu:
        ...
        db.session.commit()
        flash(u'Successfully upgrade the navigation!', 'success')
    else:
        flash(u'The navigation is at the front!', 'danger')
    return redirect(url_for('admin.manage_articleTypes_nav', page=page))
...
def nav_sort_down(id):
    ...
    if latter_menu:
        ...
        db.session.commit()
        flash(u'Successfully descending the navigation!', 'success')
    else:
        flash(u'The navigation is at the end!', 'danger')
    return redirect(url_for('admin.manage_articleTypes_nav', page=page))
...
def custom_blog_info():
    ...
    navbars = [(1, u'Charm black'), (2, u'Elegant white')]
    ...
    if form.validate_on_submit():
        ...
        flash(u'Success in modifying basic blog information!', 'success')
        return redirect(url_for('admin.custom_blog_info'))
...
def delete_plugin(id):
    ...
    try:
        ...
        db.session.rollback()
        flash(u'Delete plug-in failed!', 'danger')
    else:
        flash(u'Delete plug-in successfully!' ,'success')
...
def plugin_sort_up(id):
    ...
    if pre_plugin:
        ...
        db.session.commit()
        flash(u'Successfully upgrade the plug-in!', 'success')
    else:
        flash(u'The plug-in is already at the front!', 'danger')
    return redirect(url_for('admin.custom_blog_plugin', page=page))
...
def plugin_sort_down(id):
    ...
    if latter_plugin:
        ...
        db.session.commit()
        flash(u'Successfully descending the plug-in!', 'success')
    else:
        flash(u'The plug-in is already at the end!', 'danger')
    return redirect(url_for('admin.custom_blog_plugin', page=page))
...
def disable_plugin(id):
    ...
    db.session.add(plugin)
    db.session.commit()
    flash(u'Disable plug-ins successfully!', 'success')
    return redirect(url_for('admin.custom_blog_plugin', page=page))
...
def enable_plugin(id):
    ...
    db.session.add(plugin)
    db.session.commit()
    flash(u'Successful plug-in enablement!', 'success')
    return redirect(url_for('admin.custom_blog_plugin', page=page))
    ...
    if form.validate_on_submit():
        ...
        if plugin:
            form = AddBlogPluginForm(title=title, note=form.note.data,
                                     content=form.content.data)
            flash(u'Add plug-in failed! The plug-in name already exists.', 'danger')
            ...
            db.session.add(plugin)
            db.session.commit()
            flash(u'Add plug-in success!', 'success')
...
def edit_plugin(id):
    ...
    if form.validate_on_submit():
        ...
        if plugin_check and plugin_check.id != id:
            flash(u'Modification plug-in failed! The plug-in name already exists.', 'danger')
            ...
            db.session.add(plugin)
            db.session.commit()
            flash(u'Successful plug-in modification!', 'success')
        return redirect(url_for('admin.custom_blog_plugin', page=page))
...
def change_password():
    ...
    if form.validate_on_submit():
        if current_user.verify_password(form.old_password.data):
            ...
            db.session.commit()
            flash(u'Successful password modification!', 'success')
            return redirect(url_for('admin.account'))
        else:
            flash(u'Failed to change password! Incorrect password!', 'danger')
            return redirect(url_for('admin.account'))
...
def edit_user_info():
    ...
    if form2.validate_on_submit():
        if current_user.verify_password(form2.password.data):
            ...
            db.session.add(current_user)
            db.session.commit()
            flash(u'Successful modification of user information!', 'success')
            return redirect(url_for('admin.account'))
        else:
            flash(u'Failed to modify user information! Incorrect password!', 'danger')
            return redirect(url_for('admin.account'))
